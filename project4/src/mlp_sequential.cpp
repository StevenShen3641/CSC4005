...
{
    float *label_array_batch = new float[batch_num * class_num]; // one-hot encoded labels for the current batch
    float *fc_1_mid = new float[batch_num * hidden_num];         // multi output of fc1
    float *fc_1_out = new float[batch_num * hidden_num];         // final fc1 output with bias
    float *relu_1_out = new float[batch_num * hidden_num];       // after relu
    ...
    float *output_grad = new float[batch_num * class_num]; // gradient
    float *hidden_grad = new float[batch_num * hidden_num];
    float *loss = new float[batch_num]; // individual loss of each sample in batch
    size_t m_b = batch_num;             // mini-batch
...
void train_nn(const DataSet *train_data, const DataSet *test_data, size_t num_classes, size_t hidden_dim, size_t epochs, float lr, size_t batch)
{
    ...
    for (size_t epoch = 0; epoch < epochs; epoch++)
    {
        ...
        for (size_t offset = 0; offset < test_input_num; offset += batch)
        {
            ...
            // BEGIN YOUR CODE HERE ->
            gemm(array_batch, W1, fc1_temp, m_b, test_data->input_dim, hidden_dim);  // fc1_temp = input * W1
            add_bias(fc1_temp, fc1_temp, b1, m_b, hidden_dim); // fc1_temp = fc1_temp + b1
            Relu(fc1_temp, fc1_temp, m_b * hidden_dim);

            gemm(fc1_temp, W2, test_result + offset * num_classes, m_b, hidden_dim, num_classes);  // test_result = fc1_temp * W2
            add_bias(test_result + offset * num_classes, test_result + offset * num_classes, b2, m_b, num_classes); // test_result += b2
            // END YOUR CODE HERE <-
        }
